...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    float predicted_x = x + x_dot * dt;
    float predicted_y = y + y_dot * dt;
    float predicted_z = z + z_dot * dt;
    
    V3F accel_if = attitude.Rotate_BtoI(accel);
    
    float predicted_x_dot = x_dot + accel_if.x * dt;
    float predicted_y_dot = y_dot + accel_if.y * dt;
    float predicted_z_dot = z_dot + accel_if.z * dt - (float) CONST_GRAVITY * dt;
    
    predictedState(0) = predicted_x;
    predictedState(1) = predicted_y;
    predictedState(2) = predicted_z;
    
    predictedState(3) = predicted_x_dot;
    predictedState(4) = predicted_y_dot;
    predictedState(5) = predicted_z_dot;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    float phi = roll,       sin_phi = sin(phi),         cos_phi = cos(phi);
    float theta = pitch,    sin_theta = sin(theta),     cos_theta = cos(theta);
    float psi = yaw,        sin_psi = sin(psi),         cos_psi = cos(psi);
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    gPrime(3, 6) = accel.dot(RbgPrime(0)) * dt;
    gPrime(4, 6) = accel.dot(RbgPrime(1)) * dt;
    gPrime(5, 6) = accel.dot(RbgPrime(2)) * dt;
